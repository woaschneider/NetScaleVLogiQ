...
{
	/// <summary>
	/// Summary description for ArtikelRules.
	/// </summary>
	public partial class ArtikelRules : ABusinessRule
	{
		/// <summary>
		/// Constructor
		/// </summary>
		public ArtikelRules(ImmBusinessRuleHost hostObject) : base(hostObject)
		{
		...
		public override bool CheckRulesHook<EntityType>(mmBindingList<EntityType> entityList)
		{
			...
			if (entityList != null)
			{
				foreach (EntityType CurrentEntity in entityList)
				{
					...
					if (((Artikel)this.HostObject).IsEntityChanged(this.CurrentEntity))
					{
						ArtikelEntity Entity = CurrentEntity as ArtikelEntity;
