...
{
	/// <summary>
	/// Summary description for LagerplaetzeRules.
	/// </summary>
	public partial class LagerplaetzeRules : ABusinessRule
	{
		/// <summary>
		/// Constructor
		/// </summary>
		public LagerplaetzeRules(ImmBusinessRuleHost hostObject) : base(hostObject)
		{
		...
		public override bool CheckRulesHook<EntityType>(mmBindingList<EntityType> entityList)
		{
			...
			if (entityList != null)
			{
				foreach (EntityType CurrentEntity in entityList)
				{
					...
					if (((Lagerplaetze)this.HostObject).IsEntityChanged(this.CurrentEntity))
					{
						LagerplaetzeEntity Entity = CurrentEntity as LagerplaetzeEntity;
