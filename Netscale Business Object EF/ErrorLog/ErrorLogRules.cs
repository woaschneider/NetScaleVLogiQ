...
{
	/// <summary>
	/// Summary description for ErrorLogRules.
	/// </summary>
	public partial class ErrorLogRules : ABusinessRule
	{
		/// <summary>
		/// Constructor
		/// </summary>
		public ErrorLogRules(ImmBusinessRuleHost hostObject) : base(hostObject)
		{
		...
		public override bool CheckRulesHook<EntityType>(mmBindingList<EntityType> entityList)
		{
			...
			if (entityList != null)
			{
				foreach (EntityType CurrentEntity in entityList)
				{
					...
					if (((ErrorLog)this.HostObject).IsEntityChanged(this.CurrentEntity))
					{
						ErrorLogEntity Entity = CurrentEntity as ErrorLogEntity;
