...
{
	/// <summary>
	/// Summary description for EinstellungenRules.
	/// </summary>
	public partial class EinstellungenRules : ABusinessRule
	{
		/// <summary>
		/// Constructor
		/// </summary>
		public EinstellungenRules(ImmBusinessRuleHost hostObject) : base(hostObject)
		{
		...
		public override bool CheckRulesHook<EntityType>(mmBindingList<EntityType> entityList)
		{
			...
			if (entityList != null)
			{
				foreach (EntityType CurrentEntity in entityList)
				{
					...
					if (((Einstellungen)this.HostObject).IsEntityChanged(this.CurrentEntity))
					{
						EinstellungenEntity Entity = CurrentEntity as EinstellungenEntity;
