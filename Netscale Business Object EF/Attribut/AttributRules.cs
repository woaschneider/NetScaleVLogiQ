...
{
	/// <summary>
	/// Summary description for AttributRules.
	/// </summary>
	public partial class AttributRules : ABusinessRule
	{
		/// <summary>
		/// Constructor
		/// </summary>
		public AttributRules(ImmBusinessRuleHost hostObject) : base(hostObject)
		{
		...
		public override bool CheckRulesHook<EntityType>(mmBindingList<EntityType> entityList)
		{
			...
			if (entityList != null)
			{
				foreach (EntityType CurrentEntity in entityList)
				{
					...
					if (((Attribut)this.HostObject).IsEntityChanged(this.CurrentEntity))
					{
						AttributEntity Entity = CurrentEntity as AttributEntity;
