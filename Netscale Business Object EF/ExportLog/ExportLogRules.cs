...
{
	/// <summary>
	/// Summary description for ExportLogRules.
	/// </summary>
	public partial class ExportLogRules : ABusinessRule
	{
		/// <summary>
		/// Constructor
		/// </summary>
		public ExportLogRules(ImmBusinessRuleHost hostObject) : base(hostObject)
		{
		...
		public override bool CheckRulesHook<EntityType>(mmBindingList<EntityType> entityList)
		{
			...
			if (entityList != null)
			{
				foreach (EntityType CurrentEntity in entityList)
				{
					...
					if (((ExportLog)this.HostObject).IsEntityChanged(this.CurrentEntity))
					{
						ExportLogEntity Entity = CurrentEntity as ExportLogEntity;
