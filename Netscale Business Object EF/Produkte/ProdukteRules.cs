...
{
	/// <summary>
	/// Summary description for ProdukteRules.
	/// </summary>
	public partial class ProdukteRules : ABusinessRule
	{
		/// <summary>
		/// Constructor
		/// </summary>
		public ProdukteRules(ImmBusinessRuleHost hostObject) : base(hostObject)
		{
		...
		public override bool CheckRulesHook<EntityType>(mmBindingList<EntityType> entityList)
		{
			...
			if (entityList != null)
			{
				foreach (EntityType CurrentEntity in entityList)
				{
					...
					if (((Produkte)this.HostObject).IsEntityChanged(this.CurrentEntity))
					{
						ProdukteEntity Entity = CurrentEntity as ProdukteEntity;
